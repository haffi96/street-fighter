...
class Fighter:
    def __init__(
        self, player, x, y, flip, data, sprite_sheet, animation_steps, sound_fx
    ):
        self.player = player
        self.size = data[0]
    ...
    def move(self, screen_width, screen_height, surface, target: Self, round_over):
        SPEED = 10
        GRAVITY = 2
        dx = 0
        dy = 0
        ...
        # Get user input for keypresses
        key = pygame.key.get_pressed()

        # Can only performe actions if not attacking
    ...
    def update(self):
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            ...
            if self.alive == False:
                ...
                self.frame_index = 0
                # Check if an acttack was executed
                ...
                if self.action == 5:
                    self.hit = False
                    # if the player was the middle of an attack, then the attack is stopped
                    self.attacking = False
                    self.attack_cooldown = 20
